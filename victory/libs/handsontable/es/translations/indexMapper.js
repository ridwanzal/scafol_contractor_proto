...
function () {
  function IndexMapper() {
    ...
    this.indexToIndexCollection = new MapCollection([[INDEXES_SEQUENCE_KEY, new IndexMap()]]);
  ...
  _createClass(IndexMapper, [{
    ...
    value: function getSkippedIndexes() {
      var particularSkipsLists = arrayMap(this.skipCollection.get(), function (skipList) {
        return skipList.getValues();
      });
      var skipBooleansForIndex = pivot(particularSkipsLists);
      return arrayReduce(skipBooleansForIndex, function (skippedIndexesResult, skipIndexesAtIndex, physicalIndex) {
    ...
    value: function moveIndexes(movedIndexes, finalIndex) {
      ...
      var skippedRowsToTargetIndex = arrayReduce(this.getIndexesSequence().slice(0, indexNumber), function (skippedRowsSum, currentValue) {
